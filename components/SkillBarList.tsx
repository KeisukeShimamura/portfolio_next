...
const SkillBarList = ({ skills, title, reverse=false }: Props) => {
  ...
  return (<div className="md:w-1/2 w-full md:pl-2 my-6">
    <h3 className="text-center font-medium mb-2">{title}</h3>
    <div className="flex flex-col">
      {
        skills.map((skill) => 
          <div key={skill.id} className="relative h-10 bg-gray-100 mb-6 rounded">
            <div ref={ref} className={`${inView ? `animate-graph${skill.score}` : ''} h-10 absolute ${reverse ? 'md:right-0 md:left-auto right-auto left-0 md:flex-row-reverse flex-row' : 'left-0'} top-0 bottom-0 m-auto bg-indigo-500 flex items-center justify-between md:px-6 px-2 rounded`}>
